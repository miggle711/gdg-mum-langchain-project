...
def maybe_summarise(conversation_id: str, messages: List[BaseMessage], llm) -> tuple[Optional[str], List[BaseMessage]]:
    """If the message list exceeds the threshold, summarise older messages and trim.

    Returns (summary_text, recent_messages). If below the threshold, returns (existing_summary, all_messages).
    The caller is responsible for prepending the summary as a SystemMessage when invoking the agent.
    """
    ...
    # Keep the most recent half as verbatim context; summarise everything older
    keep = threshold // 2
