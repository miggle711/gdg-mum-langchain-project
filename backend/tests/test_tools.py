...
async def test_query_products_impl_only_includes_provided_filters(mocker, tools_module):
    mock_query = mocker.patch("tools.query_products", return_value=[])

    await tools_module.query_products_impl(category="Electronics", price_max=50)

    filters = mock_query.call_args.args[0]
    assert filters == {"category": "Electronics", "price_max": 50}


async def test_query_products_impl_formats_price_and_rating_as_strings(mocker, tools_module):
    ...
    result = json.loads(await tools_module.query_products_impl(category="Electronics"))
...
async def test_query_products_impl_handles_missing_original_price(mocker, tools_module):
    mocker.patch("tools.query_products", return_value=[{
        "id": "p1", "name": "Cable", "price": 9.99, "originalprice": None,
        "rating": 3.0, "reviews": 2, "category_name": "Electronics",
    }])

    result = json.loads(await tools_module.query_products_impl(search="cable"))

    assert result["results"][0]["original_price"] is None


async def test_query_products_impl_empty_results_returns_message(mocker, tools_module):
    mocker.patch("tools.query_products", return_value=[])

    result = json.loads(await tools_module.query_products_impl(search="nonexistent"))

    assert result["results"] == []
    assert "message" in result


async def test_query_products_impl_catches_exceptions(mocker, tools_module):
    mocker.patch("tools.query_products", side_effect=RuntimeError("ES is down"))

    result = json.loads(await tools_module.query_products_impl(search="anything"))

    assert result["results"] == []
    assert result["error"] == "ES is down"


async def test_semantic_search_impl_uses_bge_retrieval_prefix(mocker, tools_module, mock_embedding_model):
    ...
    await tools_module.semantic_search_impl("cozy winter blanket")
...
async def test_semantic_search_impl_defaults_limit_to_5_when_none(mocker, tools_module, mock_embedding_model):
    mock_embedding_model.encode.return_value = mocker.MagicMock(tolist=lambda: [0.1] * 768)
    mock_search = mocker.patch("tools.semantic_search", return_value=[])

    await tools_module.semantic_search_impl("blanket", limit=None)
...
async def test_semantic_search_impl_formats_similarity_as_percent(mocker, tools_module, mock_embedding_model):
    ...
    result = json.loads(await tools_module.semantic_search_impl("cozy blanket"))

    assert result["results"][0]["similarity"] == "87%"


async def test_semantic_search_impl_catches_exceptions(mocker, tools_module, mock_embedding_model):
    mock_embedding_model.encode.side_effect = RuntimeError("model error")

    result = json.loads(await tools_module.semantic_search_impl("blanket"))

    assert result["results"] == []
    assert result["error"] == "model error"


async def test_search_reviews_impl_uses_bge_retrieval_prefix(mocker, tools_module, mock_embedding_model):
    mock_embedding_model.encode.return_value = mocker.MagicMock(tolist=lambda: [0.1] * 768)
    mock_search = mocker.patch("tools.semantic_search_reviews", return_value=[])

    await tools_module.search_reviews_impl("battery life complaints")
...
async def test_search_reviews_impl_defaults_limit_to_5_when_none(mocker, tools_module, mock_embedding_model):
    mock_embedding_model.encode.return_value = mocker.MagicMock(tolist=lambda: [0.1] * 768)
    mock_search = mocker.patch("tools.semantic_search_reviews", return_value=[])

    await tools_module.search_reviews_impl("battery life", limit=None)

    assert mock_search.call_args.kwargs["limit"] == 5


async def test_search_reviews_impl_formats_similarity_as_percent(mocker, tools_module, mock_embedding_model):
    mock_embedding_model.encode.return_value = mocker.MagicMock(tolist=lambda: [0.1] * 768)
    ...
    result = json.loads(await tools_module.search_reviews_impl("battery life"))

    assert result["results"][0]["similarity"] == "87%"
    assert result["results"][0]["product_name"] == "Widget"


async def test_search_reviews_impl_empty_results_returns_message(mocker, tools_module, mock_embedding_model):
    mock_embedding_model.encode.return_value = mocker.MagicMock(tolist=lambda: [0.1] * 768)
    mocker.patch("tools.semantic_search_reviews", return_value=[])

    result = json.loads(await tools_module.search_reviews_impl("nonexistent"))

    assert result["results"] == []
    assert "message" in result


async def test_search_reviews_impl_catches_exceptions(mocker, tools_module, mock_embedding_model):
    mock_embedding_model.encode.side_effect = RuntimeError("model error")
...
async def test_list_categories_impl_returns_name_and_icon_only(mocker, tools_module):
    mocker.patch("tools.get_categories", return_value=[
        {"name": "Electronics", "icon": "📦", "extra_field": "ignored"},
    ])

    result = json.loads(await tools_module.list_categories_impl())

    assert result["categories"] == [{"name": "Electronics", "icon": "📦"}]


async def test_list_categories_impl_catches_exceptions(mocker, tools_module):
