...
def test_no_cached_docs_returns_none(mock_redis_binary):
    mock_redis_binary.ft.return_value.search.return_value = _search_result([])

    assert get_cached_search([0.1] * 768) is None


def test_distance_within_threshold_is_a_cache_hit(mock_redis_binary):
    # default threshold = 1.0 - 0.92 = 0.08
    mock_redis_binary.ft.return_value.search.return_value = _search_result(
        [_doc(score=0.05, results_json='[{"id": "p1"}]')]
    )

    result = get_cached_search([0.1] * 768)

    assert result == [{"id": "p1"}]


def test_distance_just_past_threshold_is_a_cache_miss(mock_redis_binary):
...
def test_set_cached_search_stores_embedding_and_results_with_ttl(mocker, mock_redis_binary):
    pipe = mocker.MagicMock()
    mock_redis_binary.pipeline.return_value = pipe

    set_cached_search("cozy blanket", [0.1] * 768, [{"id": "p1"}])
